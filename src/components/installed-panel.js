...
const DEFAULT_VIEW = {
	...
	page: 1,
	perPage: 10,
	fields: [ 'name', 'source', 'type', 'status', 'branch', 'last_updated', 'actions' ],
...
export default function InstalledPanel( {
	...
	const isConfigured =
		!! ( settings?.username || settings?.token || settings?.gitlab_token );
				...
				render: ( { item } ) => {
					...
					return (
						<Flex align="center" gap={ 1 } justify="flex-start">
							{ isGitLab ? (
								<svg aria-hidden="true" fill="#e24329" height="14" viewBox="0 0 16 16" width="14">
