	...
	const fields = useMemo(
		() => [
			...
			},
			{
				...
				render: ({ item }) => (
					<span className={`ghwp-type-badge ghwp-type-${item.type}`}>
...
function BranchSwitcherModal({ item, onClose, onSwitched, onError }) {
	...
	return (
		<Modal title={`Switch branch — ${item.repo}`} onRequestClose={onClose}>
			<ComboboxControl
...
function DeleteConfirmModal({ item, onClose, onDeleted, onError }) {
	...
	return (
		<Modal title={`Delete ${item.full_name}?`} onRequestClose={onClose}>
			<p>
				Permanently delete <strong>{item.full_name}</strong>? This will
				remove all files from the server and cannot be undone.
			</p>
